...
func (cdl *cdManagerLocal) Detach(deviceName string, nodeName types.NodeName) error {
	volumeSource, _, err := getVolumeSource(nil)
	if err != nil {
		return err
	}
...
func (cdl *cdManagerLocal) WaitForDetach(_ string, _ time.Duration) error {
...
// unmounted.
func (cdl *cdManagerLocal) UnmountDevice(_ string, mounter mount.Interface) error {
	volumeSource, _, err := getVolumeSource(nil)
	if err != nil {
		return err
	}
...
func (cdl *cdManagerLocal) CreateVolume(p *cinderVolumeProvisioner) (volumeID string, volumeSizeGB int, err error) {
	...
	if p.options.PVC.Spec.Selector != nil {
		return "", 0, errors.New("claim.Spec.Selector is not supported by Cinder for dynamic provisioning")
	}
	...
	return vol.ID, createOpts.Size, nil
}
