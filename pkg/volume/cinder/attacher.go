...
func (detacher *cinderDiskDetacher) Detach(deviceMountPath string, nodeName types.NodeName) error {
	manager, err := getManager(detacher.plugin, nil, nil)
	if err != nil {
		return err
	}
	return manager.Detach(deviceMountPath, nodeName)
}
...
func (detacher *cinderDiskDetacher) UnmountDevice(deviceMountPath string) error {
	...
	return manager.UnmountDevice(deviceMountPath, detacher.mounter)
}
