...
func (cdc *cdManagerCloud) Attach(spec *volume.Spec, nodeName types.NodeName) (string, error) {
	cloud, err := cdc.getCloudProvider()
	if err != nil {
		return "", err
	}
	...
	instanceid, err := cdc.nodeInstanceID(cloud, nodeName)
	if err != nil {
		return "", err
	}
...
func (cdc *cdManagerCloud) VolumesAreAttached(specs []*volume.Spec, nodeName types.NodeName) (map[*volume.Spec]bool, error) {
	...
	attachedResult, err := cloud.DisksAreAttached(volumeIDList, instanceid)
	if err != nil {
		// Log error and continue with attach
		glog.Errorf(
...
func (cdc *cdManagerCloud) Detach(deviceMountPath string, nodeName types.NodeName) error {
	cloud, err := cdc.getCloudProvider()
	if err != nil {
		return err
	}
...
func (cdc *cdManagerCloud) WaitForDetach(devicePath string, timeout time.Duration) error {
	ticker := time.NewTicker(checkSleepDuration)
	defer ticker.Stop()
	timer := time.NewTimer(timeout)
	defer timer.Stop()
...
func (cdc *cdManagerCloud) UnmountDevice(deviceMountPath string, mounter mount.Interface) error {
	return volumeutil.UnmountPath(deviceMountPath, mounter)
}

func (cdc *cdManagerCloud) CreateVolume(c *cinderVolumeProvisioner) (volumeID string, volumeSizeGB int, err error) {
	cloud, err := cdc.getCloudProvider()
	if err != nil {
		return "", 0, err
	}
	...
	volSizeGB := int(volume.RoundUpSize(capacity.Value(), 1024*1024*1024))
...
func (cdc *cdManagerCloud) nodeInstanceID(cloud CinderProvider, nodeName types.NodeName) (string, error) {
	instances, res := cloud.Instances()
	if !res {
		return "", fmt.Errorf("failed to list openstack instances")
	}
...
func (cdc *cdManagerCloud) getCloudProvider() (CinderProvider, error) {
	// For test purposes
	if cdc.cinderProvider != nil {
		return cdc.cinderProvider, nil
	}

	cloud := cdc.plugin.host.GetCloudProvider()
	if cloud == nil {
